...
\noindent
En este trabajo se realiza un análisis comparativo de distintas técnicas de \textbf{Aprendizaje Automático Supervisado} aplicadas a un conjunto de datos cardiológicos, con el objetivo de evaluar su desempeño en la clasificación de pacientes según riesgo o presencia de enfermedad cardíaca.\\

\noindent
En primer lugar, se lleva a cabo una \textbf{preparación del dataset}, que incluye limpieza, transformación de variables, selección de atributos y división en conjuntos de entrenamiento y prueba. Se definen las notaciones fundamentales utilizadas a lo largo del trabajo, tales como el número de muestras $n$, la cantidad de atributos $D$, el número de clases $L$, y los parámetros estimados para algunos modelos.\\
...
\begin{itemize}
	...
	\item \textbf{Máquinas de Vectores de Soporte (SVM)}, considerando márgenes duros y suaves, los hiperparámetros clave $(C, \gamma)$, el rol del vector normal $\bm{w}$ y el uso de kernels.
...
\noindent
Posteriormente, se detalla la \textbf{metodología de evaluación}, incluyendo las métricas utilizadas: exactitud, precisión, recall, especificidad, sensibilidad, F1-Score, matriz de confusión y otras métricas derivadas.\\

\noindent
A continuación, se presentan y discuten los \textbf{resultados obtenidos} para cada modelo. Se comparan sus desempeños utilizando las métricas seleccionadas.\\
...
\begin{longtable}{p{3cm} | p{10cm}}
	...
	$\bm{x}_i$ & Vector de características del registro $i$, $\bm{x}_i = (x_{i1}, \dots, x_{iD})$. \\
	$X_j$ & $j$-ésimo atributo de un registro. \\
	$C_l$ & $l$-ésima clase del conjunto de clases $\{C_1, \dots, C_L\}$. \\
	$\bm{w}$ & Vector normal al hiperplano en SVM. \\
	$\beta$ & Término de sesgo o intercepto en SVM o regresión logística. \\
	$\xi_i$ & Variable de holgura (slack) para el registro $i$ en SVM de margen suave. \\
	...
	$d'$ & Grado del polinomio en el kernel polinómico (usamos d prima para diferenciar de $D$). \\
	...
	$M_l$ & Valor de la métrica (accuracy, F1, etc.) para la clase $l$. \\
	$|y_l|$ & Cardinalidad o número de registros pertenecientes a la clase $l$. \\
...
El objetivo es describir y explicar como funcionan cada uno de los algoritmos, sus supuestos y la forma en que se aplican a los datos cardiólogos analizados.

\noindent
Un \textbf{hiperparámetro} es un parámetro cuyo valor se fija antes del proceso de entrenamiento de un modelo y que controla aspectos del aprendizaje, como la complejidad del modelo, la velocidad de convergencia o la regularización. 
...
\noindent
El modelo busca estimar la probabilidad condicional de que una observación pertenezca a la clase positiva (o clase objetivo), donde $l_i$ representa la etiqueta verdadera del registro $i$, tomando el valor $l_i = 1$ si la observación pertenece a la clase objetivo y $l_i = 0$ en caso contrario:
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Donde \(\alpha \ge 0\) es el parámetro de regularización que controla la intensidad de la penalización: valores más altos implican mayor castigo a los coeficientes. %% AR: ¿Hay alguna restricción para su valor?
			%% NS: Ahí lo agregue.
		\end{itemize}
		%{eq:l1_penalty}{}
	...
	\begin{itemize}
		...
		donde \(\boldsymbol{\beta}\) es el vector de parámetros del modelo que se actualiza en cada iteración. El método utiliza el gradiente y la inversa de la matriz Hessiana (o una aproximación de la misma mediante \emph{Conjugate Gradient}) para determinar la dirección de actualización del vector de parámetros.
		%% AR: El hessiano es el determinante de la matriz Hessiana. ¿Es esto lo que se usa o la inversa de la matriz hessiana?
		%% NS: Es la inversa el que se usa, y se intenta evitar.
		%		%{eq:newtoncg}{}
...
\noindent
De esta manera, la impureza de un nodo $t$ se mide mediante una función $\phi$ que depende de las probabilidades de clase en dicho nodo. La función de impureza $i$ es un hiperparámetro en sí mismo, y debe cumplir las siguientes condiciones:
...
\noindent
La impureza es máxima cuando las clases están perfectamente mezcladas y es mínima (cero) cuando el nodo contiene solo una clase, en donde la impureza máxima sería que la probabilidad de cada clase son aleatoria, ya que están perfectamente mezcladas.\\
...
\noindent
\textbf{Índice de Gini} cuantifica la probabilidad de clasificar incorrectamente una instancia si se asigna aleatoriamente según la distribución de clases en $t$.  
...
\noindent
donde $\widehat{y_a}(\bm{x})$ es la predicción del árbol $a$, y $A$ es la cantidad total de árboles en el bosque.\\
...
%% NS: Ahí lo corregí, despues los agrego a las notaciones.
...
\begin{itemize}
	...
	%% NS: Use las barras para la cardinaldiad.
	\item $|Y_l|$: número total de registros pertenecientes a la clase $C_l$.
...
\begin{itemize}
	...
	%% AR: falta sombrero.
	%% NS: Listo y lo puse consistente.
	
	\item $|Y_l|$: cantidad de registros cuya clase es $C_l$.
	%% AR: No es correcto.
	%% NS: Ahí lo modifique.
...
\noindent
El objetivo fundamental de una SVM es encontrar un \textbf{hiperplano de separación óptimo} que divida los datos en función de sus clases, \textbf{maximizando el margen} $\gamma$, es decir, la distancia mínima entre el hiperplano y los puntos más cercanos de cada clase (denominados \textbf{vectores de soporte}). Dichos vectores determinan la posición y orientación del hiperplano, por lo que son los únicos puntos relevantes en el entrenamiento del modelo.\\
...
%% NS: Las puse en cases, sino va avisame y las coloco como antes pero con la coma que iría.
...
%% NS: Ahí mantuve todo como hacía en los anteriroes.
...
%% AR: ¿i o l? Unificar
%% NS: Perfecto, al principio escribi todo con i pero despues decidi por l.
\begin{equation}
    \texttt{\textit{Accuracy}}(y, \hat{y}) = \frac{1}{n} \sum_{l=0}^{n_-1} 1(\hat{y}_l = y_l).
...
%% NS: Entendi a que tiene que ser mayor a 0, o agrego las diferencias de sie s menor a 1 y cuando es mayor a 1?
...
%% NS: ok, no me di cuenta que cambió adentro del entorno $$.
...
\begin{equation}
	...
	\frac{1}{\sum_{l \in L} |y_l|}
	\sum_{l \in L} |y_l| \cdot M_l.
...
%% NS: Ahí lo agregue, tengo que definir la cardinalidad  o así esta bien.
...
%% NS: Listo lo puse en todos los que vi.
\noindent
...
\text{con } w_l = \frac{|y_l|}{\sum\limits_{l=1}^{L} |y_l|}
    \label{eq:f1formula}
\end{equation}
%% AR: Corregir los límites de la sumatoria y agregar barras. Lo mismo alica para el AUC.
%% NS: Tuve que ponerle \limits para que funcione
...
%% AR: Está repetido.
%% NS: Nose si diferenciar ese l con otro subindice o supraindice.
\noindent
...
%% NS: Listo en latex, ahora lo hago en los graficos.
...
%% NS: Si me habia quedado mal eso.
...
\noindent
El tiempo de entrenamiento fue de apenas 0.60 segundos, siendo un tiempo acotado, pero costoso si vemos que el tamaño del \emph{dataset} es bastante acotado, refleja un costo de evaluación más grande que con RL.\\
...
\noindent 
El tiempo de entrenamiento fue de 1.38 segundos, siendo considerablemente el modelo en cual más se tarda en obtener los resultados.
...
\noindent
Random Forest demostró un desempeño claramente superador en la clasificación multiclase, mostrando alta capacidad de generalización incluso para un \emph{dataset} más grande y con caso multiclase.\\

...
\noindent
La cantidad de registros y los tipos de atributos utilizados para este trabajo fueron los siguientes, considerando que estos registros ya fueron previamente procesados para poder ser utilizados en las técnicas de aprendizaje automático. Se informara la distribución de los valores si el atributo es categórico, en caso de ser numérico, se informara la media de los valores, junto al valor máximo y mínimo que poseen. En la  Tabla~\ref{tab:tablebinario} se muestran que tipo de datos son los atributos del dataset que serán utilizados en este trabajo.
...
\subsection{Dataset Multiclase: Cardiotocografía Predicción}
	...
	En el dataset utilizado se procesaron automáticamente 2126 cardiotocogramas fetales (CTG) y se midieron sus características diagnósticas. Tres obstetras expertos clasificaron los CTG y se les asignó una etiqueta de clasificación consensuada. La clasificación se realizó tanto con respecto a un patrón morfológico (A, B, C...) como al estado fetal (N, S, P).\\

    \noindent
La cantidad de registros y los tipos de atributos utilizados para este trabajo fueron los siguientes, considerando que estos registros ya fueron previamente procesados para poder ser utilizados en las técnicas de aprendizaje automático. En la Tabla~\ref{tab:tablaej} se muestran los tipos de las variables utilizadas en este trabajo.
...
Las \textbf{Máquinas de Soporte Vectorial} (SVM) buscan encontrar el hiperplano que mejor separa las clases, maximizando el margen $M$ (la distancia mínima entre el hiperplano y las observaciones más cercanas, llamadas vectores de soporte). En donde estos vectores que llamamos soportes, se buscan que sean de distantas clases y tengan la distancia más cercanas posibles.
...
\end{equation}
%%{eq:hard_margin}{Problema de optimización para margen rígido}
...
\end{equation}
%%{eq:soft_margin}{Problema de optimización para margen suave}
...
\begin{equation}
	...
	\label{eq:dualsvm}
\end{equation}
%%{eq:dualsvm}{Formulación dual de SVM}
...
\end{equation}
%%{eq:kernel_lineal}{Kernel lineal}
...
\begin{equation}
	...
	\label{eq:kernel_rbf}
\end{equation}
%%{eq:kernel_rbf}{Kernel radial o gaussiano}
...
\end{equation}
%%{eq:kernel_polinomico}{Kernel polinómico de grado $d$}
...
\section{Introducción}
A continuación se mostraran los resultados obtenidos a través de los métodos previamente señalados, se detallaron los hiperparametros con mejor resultado en cuanto a lo relacionado a las métricas utilizadas. 
...
\textbf{Descripción}: Enfermedades cardiovasculares son la causa número uno de muerte globalmente, con un estimado de 17.9 millones de vidas cada año, aproximadamente 31\% de todas las muertes globales.


Este dataset~\cite{Heart_Failure}~ fue creado mediante la combinación de cinco datasets independientes en 11 atributos comunes, logrando el dataset más grande de información de enfermedades cardiovasculares utilizado para investigación. Los cinco datasets utilizados son:
...
La cantidad de registros y los tipos de atributos utilizados para este trabajo fueron los siguientes, considerando que estos registros ya fueron previamente procesados para poder ser utilizados en las técnicas de aprendizaje automático.
	...
	\underline{\textbf{Descripción atributos:}} A continuación se realizara una pequeña descripción de cada atributo anteriormente señalado. Si el atributo es categórico, se informara la distribución de los valores que posee dicho atributo y en caso de resultar ser un atributo numérico, se informara la media de los valores de dicho atributo, junto al valor máximo y mínimo que posee. Se realiza lo anterior para contextualizar los atributos y ver los rangos de valores con que se trabajara. \\
...
donde $\bm{x}_i = (x_{i1}, x_{i2}, \dots, x_{ik})$ es el vector de características de la observación $i$.
...
\begin{itemize}
	...
	\begin{equation}
		...
		\label{eq:l1_penalty}
	\end{equation}
	%{eq:l1_penalty}{Regularización L1 (Lasso).}
	...
	\begin{equation}
		...
		\label{eq:l2_penalty}
	\end{equation}
	%{eq:l2_penalty}{Regularización L2 (Ridge)}
	...
	\begin{equation}
		...
		\label{eq:elasticnet_penalty}
	\end{equation}
	%{eq:elasticnet_penalty}{Regularización Elastic Net}
	...
	\begin{itemize}
		\begin{equation}
			...
			\label{eq:newtoncg}
		\end{equation}
		%{eq:newtoncg}{Método Newton-CG (basado en segunda derivada)}
		...
		\begin{equation}
			...
			\label{eq:bfgs}
		\end{equation}
		%{eq:bfgs}{Método BFGS (quasi-Newton)}
	\end{itemize}

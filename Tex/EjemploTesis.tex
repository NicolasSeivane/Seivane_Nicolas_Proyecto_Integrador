...
\begin{longtable}{p{3cm} | p{10cm}}
	...
	$x_{ij}$ & Valor del atributo $X_j$ para el registro $i$. \\
	$y_i$ & Clase real (etiqueta verdadera) del registro $i$. \\
	$\hat{y}_i$ & Clase predicha para el registro $i$. \\
	...
	\midrule
	$\hat{\pi}_l$ & Probabilidad a priori estimada de que un registro pertenezca a la clase $C_l$. \\
	...
	$p(\bm{x}_i)$ & Probabilidad estimada de clase positiva para la observación $\bm{x}_i$ en regresión logística. \\
	...
	$C$ & Parámetro de regularización del modelo (controla el equilibrio entre ajuste y complejidad). \\
		\toprule
	...
	$q$ & Grado del polinomio en el kernel polinómico. \\
		\toprule
	\multicolumn{2}{l}{\textbf{Árboles de decisión y Random Forest}}\\
	\midrule
	$t$ & Nodo de un árbol de decisión. \\
	$N(t)$ & Número de registros que llegan al nodo $t$. \\
	...
	$\text{criterion}$ & Criterio de partición en árboles/RF (gini o entropy). \\
	$\text{max\_depth}$ & Profundidad máxima del árbol. \\
	$\text{min\_samples\_split}$ & Mínimo de muestras para dividir un nodo. \\
	$\text{min\_samples\_leaf}$ & Mínimo de muestras en una hoja. \\
	$\text{max\_features}$ & Número de atributos considerados por división. \\
	...
	$\text{Accuracy}$ & Proporción global de predicciones correctas. \\
	$\text{Precision}$ & Proporción de positivos predichos correctos, $\frac{TP}{TP+FP}$. \\
	$\text{Recall}$ & Proporción de positivos reales detectados, $\frac{TP}{TP+FN}$. \\
	$\text{F1-score}$ & Media armónica entre Precision y Recall. \\
	$F_{\beta_f}$ & F-measure ponderada que balancea Recall y Precision mediante $\beta_f$. \\
	$\text{AUC-ROC}$ & Área bajo la curva ROC. \\
...
\noindent
A continuación, se presentan y discuten los \textbf{resultados obtenidos} para cada modelo. Se comparan sus desempeños utilizando las métricas seleccionadas. El método con mejor desempeño en ambos casos es \textbf{Random Forest}, el cual obtiene valores altos en las métricas utilizadas para evaluar, siendo claro el poder del clasificador al tener varios árboles de decisión distintos que clasifican un registro, considerando la clasificación más votada. Por el contrario, se nota cómo a mayor cantidad de datos y clases para clasificar, el método tarda más cantidad de tiempo en tomar una decisión.\\
...
\subsubsection{\underline{Descripción de los atributos:}}
	...
	\noindent
	\textbf{RestingBP}: Se está describiendo la presión sanguínea en reposo, tiene un valor medio de 133.02, con un valor máximo de 200.00 y valor mínimo de 92.00.\\
	
	%{0.2cm}
	\noindent
	\textbf{Cholesterol}: Este atributo es el colesterol sérico, la medida total de colesterol en sangre; tiene un valor medio en los pacientes de 199.02, con un valor máximo de 603.00 y un valor mínimo de 85.00. Se encuentra en miligramos por decilitro. \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \(\rho \geq 0\) balancea la combinación entre L1 y L2. Si es 0 entonces tenemos L2 y si es 1 tenemos L1. 
		\end{itemize}
	...
	\begin{itemize}
		...
		\item Método SAGA: 
		Algoritmo de tipo \emph{stochastic gradient descent} con soporte para regularización L1 y L2. Permite trabajar con grandes datasets y converger de manera estable incluso con penalización L1.
...
\noindent
De esta manera, la impureza de un nodo $t$ se mide mediante una función $\phi$ que depende de las probabilidades de clase en dicho nodo. La función de impureza $\iota$ 
%% AR: Antes se usaba $i$ como subíndice, quizás haya que cambiar la notación en este caso.
%% NS: Listo use im para la impureza.
%% AR: Lo cambié por $\iota$
...
\begin{equation}
	\iota(t) = \phi \big( p(1|t), p(2|t), \ldots, p(t|t) \big).
...
\begin{itemize}
	...
	\item $q \in \mathbb{N}$: grado del polinomio 
...
\begin{equation}
	K(\bm{a}, \bm{b}) =
	(\langle \bm{a}, \bm{b} \rangle + r)^\textbf{$q$}.
...
\noindent
El \textit{F-measure} es la media armónica ponderada de \textit{precision} y \textit{recall}. La versión más común es el \textbf{F1-score}, donde el parámetro de ponderación $\beta_f$ es igual a 1, donde $\beta_f > 0$ controla la importancia relativa entre la precisión y el recall. Un clasificador perfecto tiene un valor $F1 = 1$.
\noindent
La fórmula general es:
\begin{equation}
    F_{\beta} = \frac{(1 + {\beta_f}^2) \, precision \times recall}{{\beta_f}^2 precision + recall},
    \label{eq:formulageneral}
\end{equation}

\noindent
donde la fórmula del F1-score ($\beta_f=1$) en términos de \textit{precision} y \textit{recall} se puede notar de la siguiente manera: 
...
\paragraph{Naive Bayes}{\hspace{-0.2cm}obtuvo un buen desempeño en la clasificación multiclase, considerando su supuesto de independencia entre atributos. 

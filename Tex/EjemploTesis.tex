...
\noindent
El modelo busca estimar la probabilidad condicional de que una observación pertenezca a la clase positiva (o clase objetivo), donde $l_i$ representa la etiqueta verdadera del registro $i$ (para $i=1,2, \dots,n$), tomando el valor $l_i = 1$ si la observación pertenece a la clase objetivo y $l_i = 0$ en caso contrario:
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \(\alpha\) determina cuánto se penaliza el tamaño de los coeficientes, evitando que crezcan demasiado. %% AR: ¿También es no negativo? Agregarlo.
		\end{itemize}
		...
		\begin{itemize}
			...
			\item \(\alpha\) controla el grado total de regularización; %% AR: Agregar restricciones para $\alpha$.
			\item \(\rho\) balancea la combinación entre L1 y L2. %% AR: Agregar restricciones para $\rho$.
		\end{itemize}
...
\noindent
De esta manera, la impureza de un nodo $t$ se mide mediante una función $\phi$ que depende de las probabilidades de clase en dicho nodo. La función de impureza $i$ 
...
\noindent
\textbf{NB} Gaussiano obtuvo un buen desempeño sólido en la clasificación binaria, considerando su supuesto de independencia entre atributos, que si tomamos la ida de que es \emph{buena} métrica si supera el 0.8 en su valor. 
...
\noindent Los resultados obtenidos se presentan en las Tablas~\ref{tab:rf_importancia},~\ref{tab:rl_importancia_},~\ref{tab:nb_importancia} y ~\ref{tab:svm_importancia} correspondientes a los modelos RF, RL, NB y SVM, respectivamente.
...
\noindent
En cuanto a la \textbf{importancia de las características}, se identificaron atributos dominantes en cada conjunto de datos. En el caso binario, variables como \textit{ST\_Slope}, \textit{ChestPainType} y \textit{Oldpeak} fueron recurrentemente relevantes; mientras que en el caso multiclase destacaron \textit{ASTV}, \textit{ALTV} y \textit{MSTV}.

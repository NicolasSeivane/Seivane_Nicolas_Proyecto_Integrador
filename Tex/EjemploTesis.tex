...
Para la realización de este trabajo se exploran diversas plataformas en busca de conjuntos de datos reales que resultaran relevantes para el estudio mediante técnicas de aprendizaje automático. Durante esta búsqueda se identifican múltiples \emph{datasets} de distinta naturaleza: algunos correspondientes a problemas de \textbf{clasificación binaria}, donde las observaciones se asocian a dos posibles clases, y otro \emph{datasets} de \textbf{clasificación multiclase}, con más de dos categorías posibles.\\
...
\noindent
Todos estos \emph{datasets} son procesados anteriormente a las pruebas realizadas, en donde se eliminaron tanto registros duplicados como con datos faltantes, tampoco se tienen en cuenta aquellos con datos atípicos a \textit{priori}, como un caso donde un paciente tiene colesterol 0.\\


\noindent
Se realiza una pequeña descripción de cada atributo utilizado en cada \emph{datasets}. Si el atributo es categórico, se informa la distribución de los valores que posee dicho atributo y en caso de resultar ser un atributo numérico, se informa la media de los valores de dicho atributo, junto al valor máximo y mínimo que posee. Se realiza lo anterior para contextualizar los atributos y ver los rangos de valores con que se trabajará. \\
...
\noindent
En la  Tabla~\ref{tab:tablebinario} se muestra que tipo de datos son los atributos del \emph{datasets} que son utilizados en este trabajo, considerando que estos mismos ya fueron previamente procesados para poder ser utilizados en las técnicas de aprendizaje automático. Luego la Tabla~\ref{tab:cant_datos_binarios} muestra la cantidad de registros y atributos que son utilizados, junto a que tipo de datos son.\\
...
\subsubsection{\underline{Descripción de los atributos:}}
	...
	\noindent
	\textbf{Sex}: Refiere al Sexo de los pacientes; hay una distribución de sexo de 78.98\% siendo masculinos y 21.02\%  femeninos.\\
	
	\noindent
	\textbf{ChestPainType}: Tipo del dolor en el pecho, del cual hay varias clasificaciones; Tiene una distribución 18.85\% de dolor de pecho inusual, luego un 22.11\% de dolor no de pecho, un 54.03\% asintomático, y un 5.01\% de dolor de pecho típico.\\
	...
	%{0.2cm}
	\noindent
	\textbf{Cholesterol}: Este atributo es el Colesterol serico, la medida total de colesterol en sangre; tiene un valor medio en los apcientes de 199.02, con un valor máximo de 603.00 y un valor mínimo de 0.00. Se encuentra en miligramos por decilitro. \\
	...
	%{0.2cm}
	\noindent
	\textbf{RestingECG}: Son los Resultados de electrocardiogramas en reposo; hay 60.09\% codificado en Normal, un 19.41\% codificado en ST (tiene una anormalidad en el estudio) y por último 20.50\% codificado en LVH (probablemente u a hipertrofia en el ventriculo izquierdo).\\
	
	%{0.2cm}
	\noindent
	\textbf{MaxHR}: Este atributo es el Máximo ritmo cardíaco registrado, tiene una media en los pacientes de 136.79, con un valor máximo de 202.00 y un valor mínimo 60.00. \\
	%{0.2cm}
	
	\noindent
	\textbf{ExerciseAngina}: Es la Angina producido por ejercicio, dolor en el pecho; donde hay 59.54\% que no tenían dolor codificado en N y hay 40.46\% que sí tenían dolor codificado en Y \\
	...
	%{0.2cm}
	\noindent
	\textbf{HeartDisease}: Variable de salida si posee una enfermedad cardíaca; donde hay 44.71\% que no tiene enfermedad cardíaca codificado en 0 y hay 55.29\% que si tienen enfermedad cardíaca codificado en 1.
...
\noindent
    ...
    \noindent
En la Tabla~\ref{tab:tablaej} se muestran los tipos de las variables utilizadas en este trabajo, junto con la media y valores tantos máximos como mínimos. Luego la Tabla~\ref{tab:cant_datos_multiclase} muestra la cantidad de registros y atributos que son utilizados, junto a que tipo de datos son.
...
\textbf{\underline{Descripción de los atributos:}}
	...
	%{0.2cm}
	\noindent
	\textbf{LB}: Frecuencia cardíaca fetal basal (latidos por minuto).\\
	
\noindent
	\textbf{AC}: Número de aceleraciones por segundo. \\
	
\noindent
	\textbf{FM}: Número de movimientos fetales por segundo. \\
	
\noindent
	\textbf{UC}: Número de contracciones uterinas por segundo.\\
	
\noindent
	\textbf{DL}: Número de desaceleraciones leves por segundo. \\
	
\noindent
	\textbf{DS}: Número de desaceleraciones severas por segundo.\\
	
\noindent
	\textbf{DP}: Número de desaceleraciones prolongadas por segundo.  Hay un 91.58\% con valor de 0.0, un 3.40\% con un valor 0.002, luego un 1.13\% con un valor 0.003, junto con un 3.31\% con un valor 0.001, además de un 0.43\% con un valor 0.004 y por último de un 0.14\% con un valor 0.005.\\
	
\noindent
	\textbf{ASTV}: Porcentaje de tiempo con variabilidad anormal a corto plazo.\\
	
\noindent	
	\textbf{MSTV}: Valor medio de la variabilidad a corto plazo.\\
	
\noindent
	\textbf{ALTV}: Porcentaje de tiempo con variabilidad anormal a largo plazo.  \\
	
\noindent
	\textbf{MLTV}: Valor medio de la variabilidad a largo plazo. \\
	
\noindent
	\textbf{Width}: Ancho del histograma de CTG. \\

\noindent
	\textbf{Min}: Mínimo del histograma de CTG. \\
	
\noindent
	\textbf{Max}: Máximo del histograma de CTG. \\

\noindent
	\textbf{Nmax}: Número de picos del histograma. \\
...
\noindent
	\textbf{Mode}: Moda del histograma. \\
	
\noindent
	\textbf{Mean}: Promedio del histograma. \\
	
\noindent
	\textbf{Median}: Media del histograma. \\
	
\noindent
	\textbf{Variance}: Varianza del histograma.\\
	
\noindent
	\textbf{Tendency}: Tendencia del histograma.  Hay un 39.67\% con valor de 1, 52.53\% con un valor de 0 y un 8.27\% con un valor -1.\\
...
\noindent
Para la obtención de las métricas y entrenamiento de algoritmo se utiliza la estrategia de \textbf{Validación Cruzada $k$-fold}, donde el conjunto de datos se divide en $G_i$ grupos $G:\{1, 2, 3, ..., G\} $ (o pliegues, en una traducción más fiel) del mismo tamaño, donde en cada iteración un grupo $G_i \in 1<i>K$ es utilizado para evaluar y el resto para entrenar, repitiéndose el proceso $G$ veces. Es importante señalar que un grupo $G_i$ es utilizado solo una vez para evaluar, el resto de veces será utilizado como parte del conjunto de entrenamiento. El valor final estimado de la métrica $M$, denotado por $\widehat{M}$,  es el promedio de los valores obtenidos de cada iteración, es decir,
\begin{equation}
%\label{eq:metrica}
\widehat{M} = \frac{1}{G} \sum_{i=1}^{G} M_i,
\label{eq:kfolds}
\end{equation}
donde $M_i$ es el valor de la métrica de evaluación obtenido cuando $G_i$ es utilizado como conjunto de prueba, para $i=1,2, \dots, k$.\\
...
\noindent
Una matriz de confusión, que se puede observar en la Tabla~\ref{tab:matrizconfusionbinario}, es una forma simple de saber de qué forma esté clasificando el algoritmo, donde una clase es considerada \textbf{positiva $P$} y la otra \textbf{negativa $N$}. La matriz de confusión clasifica las predicciones en:
...
\noindent
El \textit{Accuracy} es la proporción de instancias clasificadas correctamente, es una medida ``ingenua'' que puede ser engañosa si existe un gran desbalance entre clases, ergo se puede obtener un \textit{Accuracy} alto si predice una clase muy bien, que tiene una distribución mucho mayor que la otra, mientras que la de menor distribución casi no la predice. En términos de la Matriz de Confusión
\begin{equation}
    \text{\textit{Accuracy}} = \frac{TP + TN}{TP + FP + TN + FN} = \frac{TP + TN}{\text{Total}},
    \label{eq:accurcybinario}
\end{equation}
\noindent
y en términos del conjunto de predicciones y valores verdaderos, se tiene que $n_{\text{samples}}$: representa la cantidad total de ejemplos en la muestra, mientras que $\hat{y}_i$ es el valor predicho del $l$-ésimo ejemplo, e $y_l$ es el valor verdadero correspondiente, en cual caso si $\hat{y}_i$ es igual al valor obtenido, entonces se suma 1 en la sumatoria:

\begin{equation}
    \texttt{\textit{Accuracy}}(y, \hat{y}) = \frac{1}{n_\text{samples}} \sum_{l=0}^{n_\text{samples}-1} 1(\hat{y}_l = y_l),
...
\noindent
\subsection*{\textbf{\textit{Área Bajo la Curva ROC (ROC AUC)}}}


\noindent
La métrica \textit{ROC AUC} es un valor que resume la capacidad de un clasificador para distinguir entre clases, una métrica muy útil para comparar el desempeño entre modelos distintos o entre un mismo modelo con hiperparámetros distintos.\\
...
\noindent
La métrica ponderada por la presencia de la clase, $\text{M}_{\text{weighted}}$, se calcula como el promedio de la métrica por clase $M_l$, donde hay $L$ clases $\{1, 2, ..., L\}$ siendo $l \in 1< l > L$, en donde cada contribución es ponderada por el tamaño de la clase $|y_l|$, siendo $L$ es el conjunto de etiquetas o clases. Donde $\hat{M}_{\text{weighted}}$ es el valor estimado de la métrica promedio ponderada.

\begin{equation}
    \widehat{M}_{\text{weighted}} = \frac{1}{\sum_{l \in L} |y_l|} \sum_{l \in L} |y_l| \cdot \text{M}_l.
...
\begin{table}[htb]%[htb][]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\cline{3-7}
		\multicolumn{2}{c|}{} & \multicolumn{5}{c|}{\textbf{Predicción}} \\
		\cline{3-7}
		\multicolumn{2}{c|}{} 
		& \textbf{Clase $C_1$} 
		& \textbf{Clase $C_2$} 
		...
		\hline
	\end{tabular}
	\caption{Matriz Confusión Multiclase}
...
\noindent
La \textit{Precision} por clase $l$ mide la proporción de muestras clasificadas como positivas que realmente pertenecen a la clase $l$, donde $l: \{1, 2, \dots, L\}$. En términos más simple, utilizando la Matriz de Confusión:
...
\noindent
El \textit{Recall} por clase $l$, donde $l: \{1, 2, \dots, L\}$, mide la proporción de muestras verdaderamente positivas de la clase $l$ que fueron correctamente identificadas. En términos más simple, utilizando la Matriz de Confusión:
...
\noindent
El valor global ponderado se obtiene aplicando la fórmula de $M_{\text{weighted}}$ sobre los $F_{l}$, donde $\beta = 1$:

\begin{equation}
    F_{\text{weighted}} = \sum_{l \in L} w_l \, F_{l}, \quad 
\text{con } w_l = \frac{|y_l|}{\sum_{l \in L}^{L} y_l}.
    \label{eq:f1formula}
\end{equation}
...
\noindent
El modelo busca modelar la probabilidad condicional de que una observación pertenezca a la clase objetivo $L$, siendo
...
\noindent
donde $\bm{x}_i : (x_{i1}, x_{i2}, \dots, x_{ik})$ es el vector de características de la observación $i$, el cual también llamamos registro, en el cual $k$ es la cantidad de atributos que posee la observación.\\

\noindent
Por lo cual, la función logística define la \textbf{función de probabilidad} de pertenencia de $x_{i}$ a la clase $1$ como:
...
\noindent
Para solucionar este obstáculo, el calculo de los coeficientes de regresión se estiman mediante el método de \textbf{Máxima Verosimilitud} (MLE), donde la función de log-verosimilitud a maximizar es
...
\begin{itemize}
	...
	\begin{itemize}
		\item \(\beta_j\) son los coeficientes del modelo, que representan la influencia de cada variable \(x_j\) en la predicción.
		\item \(\alpha\) es el parámetro de regularización que controla la intensidad de la penalización: valores más altos implican mayor castigo a los coeficientes.
	\end{itemize}
	%{eq:l1_penalty}{}
	...
	\begin{itemize}
		...
		\item \(\alpha\) determina cuánto se penaliza el tamaño de los coeficientes, evitando que crezcan demasiado.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \(\alpha\) controla el grado total de regularización.
		\item \(\rho\) balancea la combinación entre L1 y L2.
	\end{itemize}
	...
	\begin{itemize}
		...
		\(\boldsymbol{\beta}\) es el vector de parámetros del modelo que se actualiza en cada iteración. El método usa el gradiente y el Hessiano para encontrar la dirección óptima de descenso.
%		%{eq:newtoncg}{}
...
\noindent
En consecuencia, la probabilidad de que un ejemplo en el nodo $t$ pertenezca a la clase $C_k$ se define como
...
\noindent
De esta manera, la impureza de un nodo $t$ se mide mediante una función $\phi$ que depende de las probabilidades de clase en dicho nodo, donde la función de impureza $i$ es un hiperparametro en sí mismo
...
\noindent
La impureza es máxima cuando las clases están perfectamente mezcladas y es mínima (cero) cuando el nodo contiene solo una clase, en donde la impureza máxima seria que la probabilidad de cada clase sea aleatoria, ya que están perfectamente mezcladas.
...
\noindent
El algoritmo de \textbf{Random Forest}~\cite{Breiman2001} (RF, por su equivalente en inglés \emph{Random Forest}) combina múltiples árboles de decisión independientes construidos sobre subconjuntos aleatorios de los datos (muestreo con reemplazo o \emph{bootstrap}).  
...
\noindent
El \textbf{Clasificador Naïve Bayes}~\cite{hand2001idiot} (NB) es un método supervisado probabilístico basado en el \textit{Teorema de Bayes}, que asume independencia condicional entre los atributos dado la clase. Esta suposición, aunque en la práctica rara vez se cumple estrictamente —porque las variables suelen estar correlacionadas entre sí. La regla de clasificación, o la probabilidad de pertenencia de un registro a una clase $C_k$, donde para $K$ clases $k: \{1, 2, \dots, K\}$,
...
\noindent
En donde $P(Y = C_k)$ es la \textbf{probabilidad a priori} de la clase $C_k$ (qué tan frecuente es en el conjunto de datos). Luego, $P(X_1 = x_1, \ldots, X_d = x_d \mid Y = C_k)$ es la \textbf{verosimilitud}, es decir, cuán probable es observar las características de un registro si supiéramos que pertenece a la clase $C_k$.
...
\noindent
Por lo tanto, basta estimar individualmente para cada atributo $X_j$ y cada clase $k$ la probabilidad:
\begin{equation}
	\hat{\theta}_{jmk} =
...
\item $\theta_{jmk}$: probabilidad estimada de que el atributo $X_j$ tome el valor $x_{jm}$ dentro de la clase $C_k$.
Representa “qué tan típico” es un valor dentro de una clase.

\item ${X_j = x_{jm} \wedge Y = C_k}$: cantidad de veces que aparece ese valor en esa clase.


\item ${Y = C_k}$: tamaño total de la clase $C_k$.
\end{itemize}
...
\item $\pi_k$: probabilidad estimada de que un registro pertenezca a la clase $C_k$.
Es una “creencia inicial” basada solo en la frecuencia de la clase.

\item ${Y = C_k}$: cantidad de elementos de la clase.

\item $N$: tamaño total del dataset.
\end{itemize}
...
\noindent
Pero hay un gran obstaculo, en la práctica los datos rara vez son perfectamente separables. Por ello, se introduce el concepto de \textbf{Margen Suave (Soft Margin)}, que permite violaciones controladas de las restricciones mediante variables de holgura $ \theta_i \ge 0 $. El problema se redefine como:
...
\noindent
Se busca evaluar el rendimiento de cada modelo bajo diferentes configuraciones de hiperparámetros, a través de métricas como la precisión (\textit{Accuracy}), el F1-Score y el área bajo la curva ROC (ROC AUC), entre otras.\\

\noindent
Durante la fase experimental se exploraran distintas estrategias de balanceo de clases, dado que varios de los conjuntos presentan desbalances significativos entre las clases. En particular, se prueba la técnica de \textbf{undersampling}, reduciendo la cantidad de ejemplos de la clase mayoritaria para equilibrar el \emph{datasets}. Sin embargo, esta estrategia no arroja resultados satisfactorios: los modelos tienden a perder capacidad de generalización, mostrando un descenso notable en las métricas de validación, aunque se sostiene la mejor configuración con mejor valor de métricas. Por este motivo, se opta finalmente por mantener la distribución original y aplicar técnicas de regularización y ajuste de hiperparámetros para mitigar el sesgo hacia la clase dominante.\\


\section{Dataset Binario}

\subsubsection*{Regresión Logística (RL)}

\noindent
La \textbf{Regresión Logística} obtuvo un correcto desempeño en la clasificación binaria, siempre tomando un \textbf{buen resultado} cuando cualquier métrica por lo menos supere los 0.80.\\

\noindent
Los resultados cuantitativos finales se resumen en la Tabla~\ref{tab:rl_resultados_binario}, en la cual se muestra la mejor configuración obtenida de los hiperparámetros de la Tabla~\ref{tab:grid_rl_bin}. 
El modelo logra una precisión y un F1 Score de 0.84 y 0.84 respectivamente, junto con un AUC de 0.90, lo que refleja una buena capacidad discriminatoria, pero siendo datos médicos se requieren incluso mejores valores.\\
...
La \textbf{Máquinas de Soporte Vectorial (SVM)} obtuvo un desempeño sólido en la clasificación binaria, mostrando un equilibrio adecuado entre precisión y generalización. Demostrando la solidez habitual con que es descripto este método.\\

\noindent
Los resultados cuantitativos finales se resumen en la Tabla~\ref{tab:svm_resultados_binario}. 
El modelo logra una precisión y un F1 Score de 0.86 y 0.86 respectivamente, junto con un AUC de 0.92, lo que refleja la mejor capacidad de clasificación hasta ahora.\\

\noindent
El tiempo de entrenamiento fue de apenas 0.60 segundos, siendo un tiempo acotado, pero costoso si vemos que el tamaño del \emph{dataset} es bastante acotado.\\
...
\noindent
Naive Bayes Gaussiano obtuvo un buen desempeño solido en la clasificación binario, considerando su supuesto de independencia entre atributos, que si tomamos la ida de que es \emph{buena} métrica si supera el 0.8 en su valor. La mejor configuración se alcanzó con cualquier suavizado, no hubo diferencias. Los resultados cuantitativos finales se resumen en la Tabla~\ref{tab:nbg_resultados_binario}.  El modelo logra una precisión y un F1 Score de 0.84 y 0.84 respectivamente, junto con un AUC de 0.91, lo que refleja una buena capacidad discriminativa, bastante similar con los resultados de los modelos a los cuales se compara. \\
...
\noindent
Random Forest demostró un desempeño bastante solido en la clasificación binaria, mostrando alta capacidad de generalización para registros no vistos durante el entrenamiento.\\

\noindent 
Los resultados cuantitativos finales se resumen en la Tabla~\ref{tab:rf_resultados_binario}, donde el modelo logra una precisión y un F1 Score de 0.87 y 0.87 respectivamente, junto con un AUC de 0.93, lo que refleja una buena excelente capacidad discriminatoria.\\
...
\noindent
En la Figura \ref{fig:mejores_modelos_binario} se observa que Random Forest obtiene la mayor puntuación en todas las métricas, seguido por la Regresión Logística y SVM. Esto indica que, para nuestro \emph{datasets}, el modelo de Random Forest presenta mejor capacidad de generalización, mientras que los demás modelos muestran un desempeño bastante competitivo. Esto no quiere decir que sea el mejor modelo, sino que es el que mejor resuelve este \emph{dataset}, pero si se quieren resultados sólidos pero con menor tiempo de cálculo, entonces el NB es una buena solución. Podemos afirmar que todos los modelos tuvieron desempeños correctos.
...
\noindent
La Regresión Logística obtuvo un muy buen desempeño en la clasificación multiclase, mostrando un equilibrio adecuado entre precisión y generalización.\\
...
La Máquinas de Soporte Vectorial obtuvo un desempeño bastante sólido en la clasificación multiclase, mostrando unos grandes valores en sus métricas. \\
...
El modelo logra una precisión y un F1 Score de 0.90 y 0.89 respectivamente, junto con un AUC de 0.95, lo que refleja una buena capacidad de clasificación. 
...
El modelo logra una precisión y un F1 Score de 0.82 y 0.83 respectivamente, junto con un AUC de 0.91, lo que refleja una buena capacidad clasificadora, pero lo suficientemente menor a los modelos con los cuales se compara. 
...
\noindent
Random Forest demostró un desempeño claramente superador en la clasificación multiclase, mostrando alta capacidad de generalización para registros no vistos durante el entrenamiento.\\
...
El modelo logra una precisión y un F1 Score de 0.94 y 0.94 respectivamente, junto con un AUC de 0.98, lo que refleja una buena excelente capacidad clasificadora. 

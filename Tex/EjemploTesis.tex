...
%% AR: No entiendo la última oración.
...
%% AR: Respetar el mismo estilo de tabla en todo el documento, algunas aparecen con líneas verticales y otras no.
...
%% AR: ¿Qué sería de "dolor no de pecho"?
...
%% AR: La última oración queda incompleta.
...
%% AR: Una manera más cómoda de escribir lo que sigue (tomando los dos primeros casos como ejemplo) es la siguiente:
...
\noindent
	\textbf{DP}: Número de desaceleraciones prolongadas por segundo.  Hay un 91.58\% con valor de 0, un 3.40\% con valor 0.002, luego un 1.13\% con valor 0.003, junto con un 3.31\% con valor 0.001, además de un 0.43\% con valor 0.004 y por último de un 0.14\% con valor 0.005.\\
...
\noindent
	\textbf{Nmax}: Número de picos del histograma de CTG. \\
	
\noindent
	\textbf{Nzeros}: Número de ceros del histograma de CTG.  Hay un 76.26\% con valor 0, un 17.30\% con valor 1, un 0.99\% con valor 3, un 5.11\% con valor 2, un 0.09\% con valor 4, un 0.05\% con valor 10, un 0.09\% con valor 5, un 0.05\% con valor 8 y un 0.05\% con valor 7. \\
	
\noindent
	\textbf{Mode}: Moda del histograma de CTG. \\
	
\noindent
	\textbf{Mean}: Promedio del histograma de CTG. \\
	
\noindent
	\textbf{Median}: Media del histograma de CTG. \\
	
\noindent
	\textbf{Variance}: Varianza del histograma de CTG.\\
	
\noindent
	\textbf{Tendency}: Tendencia del histograma de CTG.  Hay un 39.67\% con valor 1, un 52.53\% con un valor de 0 y un 8.27\% con un valor igual a -1.\\
...
%% AR: En el párrafo que sigue revisar la modificación que hice sobre la notación porque había inconsistencias.
\noindent
Para la obtención de las métricas y entrenamiento de algoritmo se utiliza la estrategia de \textbf{Validación Cruzada $k$-fold}, donde el conjunto de datos se divide en $K$ grupos $\{G_1, G_2, \dots, G_K\} $ (o pliegues, en una traducción más fiel) del mismo tamaño, donde en cada iteración un grupo $G_i$ ($1\le i \le K$) es utilizado para evaluar y el resto para entrenar, repitiéndose el proceso $K$ veces. Es importante señalar que un grupo $G_i$ es utilizado solo una vez para evaluar, el resto de veces será utilizado como parte del conjunto de entrenamiento. El valor final estimado de la métrica $M$, denotado por $\widehat{M}$,  es el promedio de los valores obtenidos de cada iteración, es decir,
\begin{equation}
%\label{eq:metrica}
...
\end{equation}
\noindent
En términos del conjunto de predicciones y valores verdaderos, se tiene que $n_{\text{samples}}$: representa la cantidad total de ejemplos en la muestra, mientras que $\hat{y}_i$ es el valor predicho del $l$-ésimo ejemplo, e $y_l$ es el valor verdadero correspondiente, en cual caso si $\hat{y}_i$ es igual al valor obtenido, entonces se suma 1 en la sumatoria:
%% AR: ¿i o l? Unificar
\begin{equation}
...
%% AR: En lo que sigue cambiar "Matriz de Confusión" por "matriz de confusión".
\noindent
...
\end{equation}
%% AR: Aclarar cómo debe ser el beta.
\noindent
...
%% AR: Observar que cambió el estilo para TPR y FPR. Hay que unificar.
...
\noindent
La métrica ponderada por la presencia de la clase, $M_{\text{weighted}}$, se calcula como el promedio de la métrica por clase $M_l$, donde hay $L$ clases $\{1, 2, ..., L\}$ siendo $l \in 1< l > L$, en donde cada contribución es ponderada por el tamaño de la clase $|y_l|$, siendo $L$ es el conjunto de etiquetas o clases. Donde $\widehat{M}_{\text{weighted}}$ es el valor estimado de la métrica promedio ponderada:
%% AR: Revisar notación para K-fold y corregir el párrafo anterior de manera análoga.
\begin{equation}
    \widehat{M}_{\text{weighted}} = \frac{1}{\sum_{l \in L} |y_l|} \sum_{l \in L} |y_l| \cdot M_l.
    \label{eq:metricasponderadas}
\end{equation}
%% AR: Indicar lo que representan las barras.
...
%% AR: En lo que sigue faltan los puntos suspensivos entre $C_l$ y $C_L$.
\begin{table}[htb]%[htb][]
	\centering
...
%% AR: EWn lo que sigue\, cambiar l: por l \in.
\noindent
...
\text{con } w_l = \frac{|y_l|}{\sum_{l \in L}^{L} y_l}.
    \label{eq:f1formula}
\end{equation}
%% AR: Corregir los límites de la sumatoria y agregar barras. Lo mismo alica para el AUC.

\noindent
\subsection*{\textit{Área Bajo la Curva ROC (AUC)}}
%% AR: Antes era $AUC$, elegir un estilo y unificar.
\noindent
Para extender la métrica AUC a clasificación multiclase se emplea el enfoque \textbf{One-vs-Rest (OVR)}. Para cada clase $l$, se considera la clase $l$, 
%% AR: Está repetido.
donde $l: \{1, 2, \dots, L\}$, como positiva y el resto como negativas, luego se calcula el AUC correspondiente ($\text{AUC}_l$) sobre la curva ROC de esa clasificación binaria. Finalmente, se obtiene un promedio ponderado por el soporte de cada clase:
...
%% AR: Pondría primero los modelos y luego las métricas.

\chapter{Descripción de los Métodos Utilizados}

%% AR: Agregar un breve texto que indique lo que se va a presentar en este capítulo.
...
\noindent
donde $\beta_0$ es el intercepto y $\beta_j$ son los coeficientes asociados a cada predictor, para $j=1,2,\dots,k$.\\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \(\alpha\) es el parámetro de regularización que controla la intensidad de la penalización: valores más altos implican mayor castigo a los coeficientes. %% AR: ¿Hay alguna restricción para su valor?
	\end{itemize}
	%{eq:l1_penalty}{}
	
%% AR: En lo que sigue, respetar el estilo que modifiqué para L1.
...
%% AR: El hessiano es el determinante de la matriz Hessiana. ¿Es esto lo que se usa o la inversa de la matriz hessiana?
%		%{eq:newtoncg}{}
...
\noindent
donde $N(t)$ es la cantidad total de ejemplos en el nodo $t$, y $N_k(t)$ la cantidad de ejemplos de la clase $C_k$. Es importante este fenómeno porque es un costo computacional muy barato el calcular esta probabilidad y se asemejan a las probabilidades equiprobables, donde la probabilidad de una clase en un nodo está dada por la cantidad de clases que tienen en el mismo.\\
%% AR: ¿De clases o de ejemplos?
...
%% AR: No es cualquier función, sino que debe cumplir ciertas propiedades.
\begin{equation}
...
\noindent
La impureza es máxima cuando las clases están perfectamente mezcladas y es mínima (cero) cuando el nodo contiene solo una clase, en donde la impureza máxima sería que la probabilidad de cada clase sea aleatoria, ya que están perfectamente mezcladas.
%% AR: Las probabilidades son aleatorias.
...
\noindent
El algoritmo de \textbf{Random Forest}~\cite{Breiman2001} (RF, por su equivalente en inglés \emph{Random Forest})
...
%% AR: ¿Quiénes son $\mathcal{C}$ e $\mathbb{I}$ ? Para la última antes se usó 1 (hay que unificar). Observar, además, que $M$ se usó para métrica.
...
%% AR: En los dos últimos, no creo que la notación sea RF.
\end{itemize}
...
La regla de clasificación, o la probabilidad de pertenencia de un registro a una clase $C_k$, donde para $K$ clases $k: \{1, 2, \dots, K\}$,
...
%% AR: Observar que antes se usó k para la cantidad de predictoras (o atributos), ahora se usa d. k también se usó para clases. Hay que unificar notación.
\noindent
En donde $P(Y = C_k)$ es la \textbf{probabilidad \emph{a priori}} de la clase $C_k$ (qué tan frecuente es en el conjunto de datos). Luego, $P(X_1 = x_1, \ldots, X_d = x_d \mid Y = C_k)$ es la \textbf{verosimilitud}, es decir, cuán probable es observar las características de un registro si supiéramos que pertenece a la clase $C_k$.
...
%% AR: Hay que indicar el rango en el que varía cada uno de los subíndices.
\noindent
...
%% AR: Falta el sombrero.

\item ${X_j = x_{jm} \wedge Y = C_k}$: cantidad de veces que aparece ese valor en esa clase.
%% AR: No es claro, falta el numeral y el conjunto. Observar que antes se usó barra para notar cantidad de elementos.


\item ${Y = C_k}$: tamaño total de la clase $C_k$.
%% AR: Mismo comentario que en el punto anterior.
\end{itemize}
...
%% AR: En lo que sigue, reemplazar "a priori" por \emph{a priori}.
...
\item $N$: tamaño total del \emph{dataset}.
...
%% AR: ¿Quiénes son las medias y las varianzas?
...
%% AR: ¿Quiénes son los $y_i$?

\noindent
Pero hay un gran obstáculo, debido a que en la práctica los datos rara vez son perfectamente separables. Por ello, se introduce el concepto de \textbf{Margen Suave (Soft Margin)}, que permite violaciones controladas de las restricciones mediante variables de holgura $ \theta_i \ge 0 $.
%% AR: ¿\theta o \xi?
...
\end{equation}
%% AR: Las restricciones no quedaron bien.
...
%% AR: Hay que indicar qué representan a, b, d, r y gamma.
...
%% AR: En lo anterior nunca apareció sigma.
\end{itemize}
...
%% AR: En lo que sigue, no volver a definir las siglas que ya fueron definidad previamente. simplemente, usarlas.
\subsubsection*{Regresión Logística (RL)}

\noindent
La \textbf{Regresión Logística} obtuvo un correcto desempeño en la clasificación binaria, siempre tomando un \textbf{buen resultado} cuando cualquier métrica por lo menos supere el valor de 0.80.\\
...
%% AR: En lo que sigue usar "F1-Score" en vez de "F1 Score" y "AUC" en vez de "ROC AUC". Modificar tanto en texto, como en tablas, como en figuras.
El modelo logra una precisión
%% AR: Ojo!!! Precision se definió no como accuracy. Revisar en lo que sigue.
y un F1 Score de 0.84 y 0.84 respectivamente,
%% AR: Si dan igual, no ponerlo separado.
junto con un AUC de 0.90, lo que refleja una buena capacidad discriminatoria, pero siendo datos médicos se requieren incluso mejores valores.\\
%% AR: Informar en el texto con la misma cantidad de cifras decimales que en la tabla.

%% AR: Los captions de tablas y figuras deben terminar en un punto. Revisar y corregir a partir de acá (lo anterior ya lo fui modificando).
\begin{table}[htb]%[H]
	\centering
	...
	\begin{tabular}{lcccccc}
		...
		\makecell[l]{
			...
			Penalty = l1\\
			Solver = saga \\
%% AR: saga no se mencionó previamente.
			Multiclass = ovr
...
%% AR: Primero debería ir la tabla del grid search y luego la de las métricas del mejor modelo. Respetar este orden en lo que sigue.
\begin{table}[htb]%[H]
	\centering
	\caption{Grid 
%% AR: Reemplar "grid" por "grilla" en todo el docuemnto.
de hiperparámetros - Regresión Logística (binario)}

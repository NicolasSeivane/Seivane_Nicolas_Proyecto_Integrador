...
\begin{frame}{Estado del Arte (II)}
...
\end{table}
		...
		\begin{block}{Tipos de Atributos (Resumen)}
...
\begin{frame}{Descripción de Atributos (I)}
...
\begin{frame}{Descripción de Atributos (II)}
...
\begin{frame}{Descripción de Atributos (III)}
	\begin{block}{Variables Clínicas}
		\begin{itemize} 
...
\begin{frame}{Descripción de Atributos (IV)}
...
\begin{frame}{Descripción de Atributos (V)}
	\begin{block}{Esfuerzo, Señales y Variable Objetivo}
				\begin{itemize}
...
\end{frame}
	...
	\end{frame}
		...
		\begin{block}{Criterio General}
			Se partió de los valores por defecto y se ajustaron manualmente los hiperparámetros 
			más influyentes para analizar su impacto en el desempeño, priorizando:
			\begin{itemize}
				\item reproducibilidad,
				\item consistencia comparativa,
				\item interpretabilidad del efecto de cada hiperparámetro.
			\end{itemize}
		\end{block}
...
\begin{frame}{Regresión Logística: Introducción y Motivación}
	...
	\begin{block}{¿Qué es la Regresión Logística?~\cite{cramer2002origins}}
		...
		\[
		P(l_i = 1 \mid \mathbf{x}_i)
		\]
		Es un modelo:
		\begin{itemize}
			\item Lineal en los parámetros.
			\item No lineal en la salida (usa función logística).
		\end{itemize}
	\end{block}
...
\begin{frame}{Función Logística}
	
	\begin{block}{Probabilidad Condicional}
		Para cada registro $\mathbf{x}_i = (x_{i1}, \dots, x_{iD})$:
		\[
		p(\mathbf{x}_i) =
		...
		\begin{itemize}
			\item $p(\mathbf{x}_i)$ es la probabilidad de clase positiva.
			\item $\beta_j$ mide la influencia del atributo $x_j$.
			\item El denominador asegura valores entre 0 y 1.
		\end{itemize}
	\end{block}
...
%------------------------------%
\begin{frame}{Procedimiento: Paso 1}
	\begin{block}{Iteración por atributo}
		Para cada atributo $j$ del conjunto de datos:
		\begin{enumerate}
			\item Repetir el proceso $K$ veces (para reducir la varianza de la estimación):
		\end{enumerate}
	\end{block}
\end{frame}

%------------------------------%
\begin{frame}{Procedimiento: Paso 2}
	\begin{block}{Permutación de la característica}
		Para cada repetición $k$:
		\begin{enumerate}
			\item Generar una versión alterada del conjunto de datos $X^{(k,j)}$ permutando aleatoriamente la columna $j$, manteniendo las demás columnas sin cambios.
...
%------------------------------%
\begin{frame}{Procedimiento: Paso 3}
	\begin{block}{Cálculo de importancia}
...
\begin{frame}{Interpretación}
	\begin{block}{Significado de $I_j$}
...
\end{frame}
	...
	\begin{frame}{Resultados Principales}
		\begin{itemize}
			\item Se evaluaron todos los modelos sobre el mismo conjunto de datos.
			\item Se compararon métricas como Accuracy, F1 y AUC.
			\item Los resultados muestran diferencias claras según el modelo:
			\begin{itemize}
				\item Algunos modelos lineales funcionan bien en escenarios simples.
				\item Modelos como Random Forest presentan mejor estabilidad y mayor AUC.
				\item Naïve Bayes destaca por su velocidad.
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Análisis e Interpretación}
		\begin{itemize}
			\item Los modelos basados en árboles (Random Forest) tienden a capturar relaciones no lineales complejas.
			\item SVM proporciona buenas fronteras de decisión cuando se emplean kernels adecuados.
			\item Naïve Bayes funciona sorprendentemente bien incluso con suposiciones fuertes.
			\item Regresión Logística es la más interpretable y útil como línea base.
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Conclusiones}
		\begin{itemize}
			\item Los modelos supervisados pueden asistir de manera efectiva en el análisis clínico.
			\item Es fundamental seleccionar un modelo considerando:
			\begin{itemize}
				\item Interpretabilidad.
				\item Métricas de desempeño.
				\item Costo computacional.
			\end{itemize}
			\item Random Forest y SVM presentan ventajas claras en contextos médicos.
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Trabajo Futuro}
		\begin{itemize}
			\item Evaluar modelos más avanzados (Gradient Boosting, XGBoost, Redes Neuronales).
			\item Incluir mayor cantidad de datos clínicos heterogéneos.
			\item Agregar interpretabilidad avanzada (SHAP, LIME).
			\item Optimizar hiperparámetros mediante búsqueda bayesiana.
		\end{itemize}
	\end{frame}

...
%\end{frame}
	...
	\begin{frame}{Metodología General}
		\begin{block}{}
		\begin{itemize}
	\item \emph{Dataset} clínico con múltiples variables relacionadas al estado cardíaco.
...
\end{itemize}
	...
	% ----------------------------------------------------
	% DATASET
	\begin{frame}{\emph{Dataset} Binario: \textit{Insuficiencia Cardíaca Predicción}}
		\begin{block}{Origen del \emph{Dataset}}
			
			El \emph{dataset} \textit{HeartFailure}~\cite{HeartFailure} se construyó combinando cinco conjuntos de datos cardiovasculares, unificados en 11 atributos comunes. Esto permite obtener uno de los conjuntos más grandes usados en investigación clínica.
			
			\vspace{0.2cm}
			
			\textbf{\emph{Datasets} incluidos (cantidad de registros aportados):}
			\begin{itemize}
				\item Cleveland (303)
				\item Hungarian (294)
...
\begin{frame}{\emph{Dataset} Binario: Características Generales}
		\begin{block}{}
...
\begin{frame}{\emph{Dataset} Multiclase: \emph{Cardiotocografía Predicción}}
	\begin{block}{Origen del \emph{Dataset}}
	...
	\begin{block}{Variable objetivo}
		...
		La variable objetivo es \textbf{CLASS}, que clasifica el estado fetal en 
		\emph{normal (N)}, \emph{sospechoso (S)} o \emph{patológico (P)}.
...
\end{frame}
	...
	\begin{frame}{\emph{Dataset} Multiclase: Características Generales}
	\begin{block}{}
...
\begin{block}{Tipos de Atributos}
	...
	El conjunto de datos incluye variables relacionadas con la \textbf{frecuencia cardíaca fetal}, 
	las \textbf{aceleraciones y desaceleraciones}, los \textbf{movimientos fetales}, la 
	\textbf{actividad uterina}, así como medidas de \textbf{variabilidad a corto y largo plazo} 
	(ASTV, MSTV, ALTV, MLTV). 
...
\begin{frame}{Cardiotocografía}
	
	\begin{block}{Registro CTG}
		\centering
		\begin{figure}[htb]
			\centering
			\includegraphics[width=0.7\textwidth]{../cardiotografia}
			\caption{Ejemplo de registro de cardiotocografía (CTG). Fuente: Wikipedia.}
		\end{figure}
	\end{block}
...
%\end{frame}
	...
	\begin{frame}{Atributos del \emph{Dataset} Binario}
...
\end{frame}
	...
	\begin{frame}{Atributos del \emph{Dataset} Multiclase}
	
	\begin{block}{Tipo de atributo del conjunto multiclase.}
\tiny
...
%	\end{frame}
	...
	\begin{frame}{\emph{Accuracy}}
		\begin{block}{Definición}
			Proporción de instancias correctamente clasificadas:
			\[
			\text{\emph{Accuracy}} = \frac{TP + TN}{TP + FP + TN + FN} = \frac{TP + TN}{\text{Total}}
			\]
		\end{block}
		
		\begin{block}{Conjunto de predicciones}
			\[
			\text{\textit{\emph{Accuracy}}}(y, \hat{y}) = \frac{1}{n} \sum_{l=0}^{n-1} \mathbb{I}(\hat{y}_l = y_l), \quad
		...
		\begin{block}{Resumen}
			\[
			\text{\emph{Accuracy}} = \frac{\text{Número de predicciones correctas}}{\text{Número total de muestras}}
			\]
		\end{block}
	\end{frame}
	...
	\begin{frame}{\emph{Recall} / Sensibilidad}
		\begin{block}{Definición}
			Mide la probabilidad de detectar un caso positivo real:
			\[
			\text{\emph{Recall}} = \text{TPR} = \frac{TP}{TP + FN} = \frac{TP}{P}
			\]
		\end{block}
	\end{frame}
	...
	\begin{frame}{F-measure / F1-score}
		\begin{block}{Definición general}
			Media armónica ponderada de precisión y \emph{recall}:
			\[
			F_{\beta} = \frac{(1 + \beta_f^2) \, \text{precisión} \cdot \text{\emph{recall}}}{\beta_f^2 \text{precisión} + \text{\emph{recall}}}
			\]
		\end{block}
		
		\begin{block}{F1-score ($\beta_f=1$)}
			\[
			F1 = 2 \cdot \frac{\text{precisión} \cdot \text{\emph{recall}}}{\text{precisión} + \text{\emph{recall}}}
			\]
			\[
...
\pause
	\begin{block}{Caso Multiclase}
En el caso multiclase, las métricas de evaluación se definen de manera análoga al caso binario.
En situaciones particulares, la clasificación se implementa mediante el enfoque uno contra todos (\emph{one-vs-rest}), en el cual se entrena un modelo por cada clase considerando a dicha clase como positiva y al resto como negativa.
\end{block}
...
\begin{frame}{Naive Bayes – Resultados en el caso multiclase}
...
%---------------------------------------------------------
\begin{frame}{Regresión Logística – Resultados en el caso binario}
...
\begin{frame}{Regresión Logística – Resultados en el caso multiclase}
...
%---------------------------------------------------------
\begin{frame}{Random Forest – Resultados en el caso binario}
...
\begin{frame}{Random Forest – Resultados en el caso multiclase}
...
%---------------------------------------------------------
\begin{frame}{SVM – Resultados en el caso binario}
...
\begin{frame}{SVM – Resultados en el caso multiclase}
	
	\begin{block}{Comportamiento del modelo}
		\begin{itemize}
			\item Alta capacidad discriminatoria.
			\item Kernel Polinomico fue el de mejor desempeño, por lo cual fue necesario cambiar la dimensionalidad para encontrar el hiperplano.
			\item Tiempo de entrenamiento moderado.
		\end{itemize}
	\end{block}
...
%---------------------------------------------------------
\begin{frame}{Comparación de los mejores modelos en el caso binario}
...
\begin{frame}{Comparación de los mejores modelos en el caso multiclase}
	
	\begin{block}{Conclusiones por desempeño}
		\small
		\begin{itemize}
			\item Se observa que Random Forest obtiene la mayor puntuación en todas las métricas, seguido por SVM.
		\end{itemize}
	\end{block}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.7\textwidth]{../mejores_modelos_barras_multiclase}
...
\begin{frame}{Importancia de Características – Permutation Importance}
	...
	\begin{block}{Hallazgo general}
		\textbf{ST\_Slope}, \textbf{Oldpeak} y \textbf{ChestPainType} aparecen como la características más relevantes en los cuatro modelos en caso binario.\\
		\textbf{ASTV}, \textbf{ALTV} y \textbf{MSTV} aparecen como la características más relevantes en los cuatro modelos en caso multiclase.
...
\only<2>{	
	...
	\begin{block}{Importancia de las características para NB en el caso multiclase.}
			\centering
			\begin{figure}[htb]%[H]
				\centering
				\includegraphics[width=0.7\textwidth]{../permutacion_nb_multi}
			\end{figure}
		\end{block}
...
\only<4>{	
	...
	\begin{block}{Importancia de las características para RL en el caso multiclase.}
			\centering
			\begin{figure}[htb]%[H]
				\centering
				\includegraphics[width=0.7\textwidth]{../permutacion_rl_multi}
			\end{figure}
		\end{block}
...
\only<6>{	
	...
	\begin{block}{Importancia de las características para RF en el caso multiclase.}
			\centering
			\begin{figure}[htb]%[H]
				\centering
				\includegraphics[width=0.7\textwidth]{../permutacion_rf_multi}
			\end{figure}
		\end{block}
...
\only<8>{	
	...
	\begin{block}{Importancia de las características para SVM en el caso multiclase.}
			\centering
			\begin{figure}[htb]%[H]
				\centering
				\includegraphics[width=0.7\textwidth]{../permutacion_svm_multi}
			\end{figure}
		\end{block}
...
%---------------------------------------------------------
\begin{frame}{Rendimiento del Rendimiento por Número de Características}
...
\begin{frame}{Rendimiento al incorporar métricas en el caso binario.}
...
\begin{frame}{Rendimiento al incorporar métricas en el caso multiclase.}
	...
	\only<4>{
		\begin{block}{Naive Bayes}
			\begin{itemize}
				...
				\item Muy competitivo como modelo inicial y para \emph{datasets} pequeños.
			\end{itemize}
		\end{block}
	}

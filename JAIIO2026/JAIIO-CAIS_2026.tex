...
Se concluye que el modelo RF es la herramienta más robusta para el tipo de datos bajo análisis debido a su capacidad para capturar relaciones no lineales y reducir el sobreajuste mediante el voto mayoritario de los árboles que lo integran. SVM también mostró un rendimiento competitivo, especialmente con \emph{kernels} radiales y polinómicos. El estudio permitió identificar atributos críticos, como la pendiente del segmento ST en el ECG y el porcentaje de tiempo con variabilidad anormal a corto plazo, para el diagnóstico de riesgo cardíaco. 
Como trabajos futuros, se propone el uso de optimización bayesiana para un ajuste de hiperparámetros más fino y técnicas de reducción de dimensionalidad, como \emph{Principal Component Analysis}, para simplificar los modelos sin perder precisión.

Esta línea de trabajo se basa en la hipótesis de que el uso de herramientas de Aprendizaje Automático en entornos sanitarios puede ser útil en la asistencia a los profesionales de ciencias de la salud, tanto al momento de realizar el diagnóstico de una patología cardíaca, como en el diseño de estudios capaces de capturas variables que resulten de importancia para el entrenamiento de un algoritmo, favoreciendo incluso prácticas no invasivas para los pacientes.

%Asimismo, se plantea la posibilidad de potenciar este soporte de diagnóstico siempre que exista información clínica previa disponible, entendiendo al modelo no como una solución aislada, sino como una vía avanzada para relacionar e integrar datos del paciente de froma màs ágil. Bajo este enfoque, el output generado se concibe como un eslabón modular dentro de una cadena más amplia, capaz de integrarse a sistemas complejos de predicción y clasificación médica. No obstante, la posibilidad de acierto de esta arquitectura depende estrictamente de la gobernanza de los datos, identificando como principal riesgo metodológico la presencia de información mal cargada, errónea o desactualizada.
